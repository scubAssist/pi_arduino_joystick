...
const byte button_pins[]={2,3,4,5,6,7,8,9,11,12,13};

#define axis_number 6
const byte axis_pins[]={0,1,2,3,4,5};
...
void loop(){
  ...
  */
  ...
  // REAL LOOP
  while(true) {
    // Read the analog sticks and buttons
    for(int b=0; b<button_number; b++){
    ...
    for(int a=0; a<axis_number; a++){
      float axisValue = analogRead(axis_pins[a]);
      // TODO, determine mapping range output
      axisValue = map(axisValue, 0, 1024, -100, 100);
      Serial.print('A');
      Serial.print(a);
      char buffer [4];
      sprintf(buffer, "%03d", axisValue);
      Serial.print(":");
      Serial.println(buffer);
    }

    // Read from serial to get ppm vals
    readLineFromSerial();
    if (0 == strcmp(cmd, "ppm")) {
